...
ggplot(boxplot_type, aes(name_f,weather))+
  ...
  scale_fill_gradientn(colours=c("red","white","green"),name="Price")+
  labs(title ="Uber VS Lyft: Heat Map for Product types and Weather", x = "Product types", y = "Weather")+
  theme_bw()+facet_wrap(~cab_type)

ggplot(boxplot_type, aes(price,weather))+
  geom_boxplot(aes(fill = factor(cab_type)))+
  scale_fill_manual(values=c("#ea0b8c", "black"),name=" ")+
  labs(title ="Uber VS Lyft: Boxplot for Price and Weather", x = "Price", y = "Weather")+
  theme_bw()+facet_wrap(~cab_type)
